...
def test_manual_is_keybase_verified_ok(api_client, user, gpg_key, gpg, factories):
    keybase_verify_url = reverse('v1:verify-keybase-detail', args=[user.pk])
    ...
    response_data = {
        'keybase_verified': False,
    }

    response = api_client.get(keybase_verify_url)
    assert response.status_code == status.HTTP_200_OK, response.data
    assert response.data['keybase_verified'] == response_data['keybase_verified']


def test_manual_is_keybase_verified_ok_admin(api_client, user, gpg_key, gpg, factories):
    keybase_verify_url = reverse('v1:verify-keybase-detail', args=[user.pk])
    user_admin = factories.UserFactory(is_staff=True)
    api_client.force_authenticate(user_admin)
...
def test_manual_is_keybase_verified_deny(api_client, user, gpg_key, gpg, factories):
    new_user = factories.UserFactory()
    keybase_verify_url = reverse('v1:verify-keybase-detail', args=[new_user.pk])
